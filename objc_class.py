...
#        print ('%08x: Parsing properties' % ea)
        entry_size = idc.get_wide_dword(ea)
        num_entries = idc.get_wide_dword(ea + 4)

        ea = ea + 8
        for i in range(num_entries):
            var_name = idc.get_strlit_contents(idc.get_wide_dword(ea), -1, idc.ASCSTR_C)
            var_type = idc.get_strlit_contents(idc.get_wide_dword(ea + 4), -1, idc.ASCSTR_C)
...
#        print ('%08x: Parsing ivars' % ea)
        ...
        ea = ea + 8
        for i in range(num_entries):
            ivar_offset = idc.get_wide_dword(idc.get_wide_dword(ea))
            ivar_name = idc.get_strlit_contents(idc.get_wide_dword(ea + 4), -1, idc.ASCSTR_C)
            ivar_type = idc.get_strlit_contents(idc.get_wide_dword(ea + 8), -1, idc.ASCSTR_C)
...
#        print ('%08x: Parsing methods' % ea)
        ...
        ea = ea + 8
        for i in range(num_entries):
            method_name = idc.get_strlit_contents(idc.get_wide_dword(ea), -1, idc.ASCSTR_C)
            method_type = idc.get_strlit_contents(idc.get_wide_dword(ea + 4), -1, idc.ASCSTR_C)
            method_ea = idc.get_wide_dword(ea + 8)
...
#        print ('%08x: Parsing protocols' % ea)
        num_main_structs = idc.get_wide_dword(ea)
        ea = ea + 4

        for i in range(num_main_structs):
            struct_off = idc.get_wide_dword(ea)
            protocol_name = idc.get_strlit_contents(idc.get_wide_dword(struct_off + 4), -1, idc.ASCSTR_C)
            protocol_list = idc.get_wide_dword(struct_off + 8)
            instance_methods = idc.get_wide_dword(struct_off + 0xC)
            class_methods = idc.get_wide_dword(struct_off + 0x14)
...
class ObjcClass:
    def __init__(self, ea):
        ...
        objc_const_seg = idc.get_segm_by_sel(idc.selector_by_name('__objc_const'))
        objc_const_end = idc.get_segm_end(objc_const_seg)

        self.class_info['meta_class'] = idc.get_wide_dword(ea)
        self.class_info['super_class'] = idc.get_wide_dword(ea + 4)
        self.class_info['cache'] = idc.get_wide_dword(ea + 8)
        self.class_info['vtable'] = idc.get_wide_dword(ea + 0xC)
        _class_def = idc.get_wide_dword(ea + 0x10)

        if _class_def < objc_const_seg or _class_def > objc_const_end:
            return

        self.class_info['top_class'] = idc.get_wide_dword(_class_def)
        self.class_info['instance_size'] = idc.get_wide_dword(_class_def + 8)

        name_off = idc.get_wide_dword(_class_def + 0x10)
        class_name = idc.get_strlit_contents(name_off, -1, idc.ASCSTR_C)
        if not class_name:
            class_name = '[UNKNOWN]'
        ...
        if idc.get_wide_dword(_class_def + 0x14):
            self.class_info['methods'] = ObjcMethods(idc.get_wide_dword(_class_def + 0x14))

        if idc.get_wide_dword(_class_def + 0x18):
            self.class_info['protocols'] = ObjcProtocols(idc.get_wide_dword(_class_def + 0x18))

        if idc.get_wide_dword(_class_def + 0x1C):
            self.class_info['ivars'] = ObjcIvars(idc.get_wide_dword(_class_def + 0x1C))

        if idc.get_wide_dword(_class_def + 0x24):
            self.class_info['properties'] = ObjcProperties(idc.get_wide_dword(_class_def + 0x24))
...
def main():
    objc_data_seg = idc.get_segm_by_sel(idc.selector_by_name('__objc_data'))
    if objc_data_seg == idc.BADADDR:
        print('Cannot locate objc_data segment')
        return

    ea = objc_data_seg
    while ea < idc.get_segm_end(objc_data_seg):
